...
if (!empty($_POST)) {
    if (isset($_POST['from'])) {
        ...
        {
            if (isset($_POST['kill'.$pub->id]))
            {
                $conn::$Bd->query('DELETE FROM votes WHERE publication='.$pub->id.';');
                $conn::$Bd->query('DELETE FROM publications WHERE id='.$pub->id.';');
                unset($_POST['kill'.$pub->id]);
